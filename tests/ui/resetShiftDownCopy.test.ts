...
describe("isPrimaryShiftDown — true only when SHIFT DOWN is the actual Engine recommendation", () => {
  it("is true for a RED (STABILIZE) recommendation", () => {
    const rec = evaluate({ beyondDayId: "d", checkIn: checkIn({ stress: 5 }), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
  ...
  it("is true for a POST_SHIFT_TRANSITION recommendation (Drop 02b)", () => {
    const rec = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: false, hasUnresolvedPostShift: true, hasEligibleObligationDueOrOverdue: false });
    expect(rec.kind).toBe("POST_SHIFT_TRANSITION");
    expect(isPrimaryShiftDown(rec)).toBe(true);
  });

  it("is false for RECOVER (YELLOW)", () => {
    const rec = evaluate({ beyondDayId: "d", checkIn: checkIn({ soreness: 4 }), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
  ...
  it("is false for EXECUTE_PLANNED_WORK (GREEN + planned work)", () => {
    const rec = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: true, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
    expect(rec.kind).toBe("EXECUTE_PLANNED_WORK");
    expect(isPrimaryShiftDown(rec)).toBe(false);
  });

  it("is false for NO_ACTION_REQUIRED", () => {
    const rec = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
...
describe("isPrimaryReset — no engine path produces this today, verified against every real recommendation kind", () => {
  it("is false for all six kinds the locked engine can actually issue", () => {
    const stabilize = evaluate({ beyondDayId: "d", checkIn: checkIn({ stress: 5 }), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
    const postShiftTransition = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: false, hasUnresolvedPostShift: true, hasEligibleObligationDueOrOverdue: false });
    const recover = evaluate({ beyondDayId: "d", checkIn: checkIn({ soreness: 4 }), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
    const executePlannedWork = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: true, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });
    const obligationDue = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: true });
    const noActionRequired = evaluate({ beyondDayId: "d", checkIn: checkIn({}), hasPlannedWork: false, hasUnresolvedPostShift: false, hasEligibleObligationDueOrOverdue: false });

    for (const rec of [stabilize, postShiftTransition, recover, executePlannedWork, obligationDue, noActionRequired]) {
